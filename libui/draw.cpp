...
static ID2D1GradientStopCollection *mkstops(uiDrawBrush *b, ID2D1RenderTarget *rt)
{
	...
	stops = (D2D1_GRADIENT_STOP *) libui_alloc(b->NumStops * sizeof (D2D1_GRADIENT_STOP), "D2D1_GRADIENT_STOP[]");
...
void uiDrawStroke(uiDrawContext *c, uiDrawPath *p, uiDrawBrush *b, uiDrawStrokeParams *sp)
{
	...
	if (sp->NumDashes != 0) {
		...
		dashes = (FLOAT *) libui_alloc(sp->NumDashes * sizeof (FLOAT), "FLOAT[]");
