...
WCHAR *toUTF16(const char *str)
{
	...
	wstr = (WCHAR *) libui_alloc((n + 1) * sizeof (WCHAR), "WCHAR[]");
	wp = wstr;
	while (*str) {
...
char *toUTF8(const WCHAR *wstr)
{
	...
	str = (char *) libui_alloc((n + 1) * sizeof (char), "char[]");
	sp = str;
	while (*wstr) {
...
WCHAR *utf16dup(const WCHAR *orig)
{
	...
	len = wcslen(orig);
	out = (WCHAR *) libui_alloc((len + 1) * sizeof (WCHAR), "WCHAR[]");
...
WCHAR *vstrf(const WCHAR *format, va_list ap)
{
	...
	buf = (WCHAR *) libui_alloc(n * sizeof (WCHAR), "WCHAR[]");
...
char *LFtoCRLF(const char *lfonly)
{
	...
	len = strlen(lfonly);
	crlf = (char *) libui_alloc((len * 2 + 1) * sizeof (char), "char[]");
	out = crlf;
	for (i = 0; i < len; i++) {
		if (*lfonly == '\n')
			*crlf++ = '\r';
